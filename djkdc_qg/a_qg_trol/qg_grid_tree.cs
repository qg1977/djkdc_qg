    ...
    public partial class qg_grid_tree : DataGridView
    {
        ...
        private void qg_grid_tree_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                string id = Rows[e.RowIndex].Cells["编码"].Value.ToString();

                DataRow[] dtrow = dt.Select("编码='" + id + "'");
                //MessageBox.Show(dtrow.Count().ToString());
                ...
                if (zk == 1)
                {
                    ...
                    dtrow[0][0] = dtrow[0][0].ToString().Replace("○", "●");
                    for (int i = 0; i < dtrow_1x.Count(); i++) { dtrow_1x[i]["显示"] = 0; /*MessageBox.Show(dtrow_1x[i]["ID"].ToString()); */}
                    ...
                    for (int i = 0; i < dtrow_1.Count(); i++)
                    {
                        ...
                        dtrow_1[i][0] = dtrow_1[i][0].ToString().Replace("○", "●");
                    }
                }
                else
                {
                    //MessageBox.Show(dtrow[0]["ID"].ToString());
                    dtrow[0]["展开"] = 1;
                    //dtrow[0]["部门名称"] = dtrow[0]["部门名称"].ToString().Replace( "●", "○");
                    dtrow[0][0] = dtrow[0][0].ToString().Replace("●", "○");
