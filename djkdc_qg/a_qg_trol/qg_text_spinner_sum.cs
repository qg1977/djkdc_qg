...
{
    //只能输入纯数字，1234567890
    public partial class qg_text_spinner_sum : TextBox
    {
        public qg_text_spinner_sum()
        {
            InitializeComponent();
            this.SuspendLayout();
        ...
        private void qg_text_spinner_sum_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                //判断按键是不是要输入的类型。
        ...
        private void qg_text_spinner_sum_Enter(object sender, EventArgs e)
        {
            ImeMode = ImeMode.Disable;//获得焦点时,设置为英文
            BackColor = Color.LightCyan; //当textBox1获得焦点时，背景色变为LightCyan（淡蓝绿色）
        }

        private void qg_text_spinner_sum_Leave(object sender, EventArgs e)
        {
