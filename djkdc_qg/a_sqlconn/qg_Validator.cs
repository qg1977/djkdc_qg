...
namespace djkdc_qg.a_sqlconn
{
    class qg_Validator
    {
        ...
        /// <summary>  
        /// 验证密码格式是否正确
        /// 6~20位英文字母、数字或常用英文符号组成， 常用英文字符为  `~!@#$%^&*()_+-={}|[]\\:\";'<>?,./     
        /// </summary>  
        /// <param name="strln">输入的字符</param>  
        /// <returns></returns>  
        public static bool IsPassJytt(string strln)
        {
            //return Regex.IsMatch(strln, @"^[0-9a-zA-Z`~!@#$%\^&*()_+-={}|\[\]:"";'<>?,.]{6,20}$");
            return Regex.IsMatch(strln, @"^[0-9a-zA-Z`~!@_]{5,20}$");
        }
